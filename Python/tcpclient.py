...
class SCTCPClient:
    ...
    def send_message(self, method_id, message, error_id=0):
        ...
        data = str(method_id)+'/'+str(message_id)+'/'+str(error_id)+'/'+str(message)
    ...
    def send_table(self, tableName, rowCount):
        with self.sqldb:
             ...
             if (len(rows)>0):
                ...
                for val in rows:
                    id = val['id']
                    st = "update  "+tableName+" set synchro='"+message_id+" "+datetime.strftime(datetime.now(), "%Y.%m.%d %H:%M:%S")+"' where id="+str(id)
                    #print(st)
                    cur.execute(st)
                    #print(rows[1]['id'])
