...
/**
 * https://github.com/SeinopSys/PHP-PostgreSQL-Database-Class/blob/master/USAGE.md
 */
...
class PostgresDb
{
    protected
        ...
        /**
         * List of keyords used for escaping column names, automatically populated on connection
         */
        $_sqlKeywords = [];
    ...
    public function __construct($db, $host, $port, $user, $pass)
    {
        $this->_connstr = "pgsql:host=$host;port=$port;dbname=$db;user=$user;password=$pass;";
    }
    ...
    {
        $this->setConnection(new PDO($this->_connstr));
    ...
    protected function _buildJoin()
    {
        ...
        foreach ($this->_join as list($joinType, $joinTable, $joinCondition)) {
    ...
    protected function _buildWhere()
    {
        ...
        foreach ($this->_where as list($cond, $whereProp, $operator, $whereValue)) {
     ...
     *
     * @return boolean|mixed Boolean indicating whether the insert query was completed succesfully.
     ...
     * Abstraction method that will build the LIMIT part of the WHERE statement
     *
     * @param int|int[] $numRows An array to define SQL limit in format array($limit,$offset) or just $limit
     */
     ...
     *
     * @param int|int[] $numRows Array to define SQL limit in format array($limit,$offset) or just $limit
    ...
    protected function _buildQuery($numRows = null, $tableData = null, $returning = null)
    {
        ...
        if ($returning !== null) {
            ...
            foreach ($returning as $column) {
                $this->_query .= $this->_quoteColumnName($column, true);
            }
        }
        $this->_buildGroupBy();
        $this->_buildOrderBy();
     ...
     * @throws PDOException
     */
    public function querySingle(...$args)
    {
        return $this->_singleRow($this->query(...$args));
    }
    ...
    protected function _quoteColumnName($columnName, $allowAs = false)
    {
        ...
        if (!preg_match('~(^\w+\(|^\s*\*\s*$)~', $columnName) && (!preg_match('~^(?=[a-z_])([a-z\d_]+)$~', $columnName) || isset($this->_sqlKeywords[ strtolower($columnName) ]))) {
    ...
    public function tableNameToClassName($namespaced = false)
    {
        ...
        if (is_string($className)) {
            $className = preg_replace('/s(_|$)/', '$1', preg_replace('/ies([-_]|$)/', 'y$1', preg_replace_callback('/(?:^|-)([a-z])/', function ($match) {
