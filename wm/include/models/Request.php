...
class Request
{
    ...
    public static function parseRequest($conn,$querystring){
     ...
     if(!$conn || !$querystring){ return false;}else{
        ...
        //     $msg = Validator::str_decrypt($querystring);
        //     $querystring = Request::bin2hex($querystring);
        ...
        //     if(!is_array($msg_arr) || !count($msg_arr)){ return false;}
        //     var_dump($msg_arr);
        //     var_dump(Request::bin2hex($msg_arr[3]));
            
        //     if(isset($msg_arr[0]) && ctype_xdigit($msg_arr[0])){ $q['method_id'] = Validator::hex_to_int($msg_arr[0]);}else{ return false;}
        //     if(isset($msg_arr[1]) && ctype_xdigit($msg_arr[1])){ $q['message_id'] = Validator::hex_to_int($msg_arr[1]);}else{ return false;}
        //     if(isset($msg_arr[2]) && ctype_xdigit($msg_arr[2])){ $q['error_id'] = Validator::hex_to_int($msg_arr[2]);}else{ return false;}
        //     if($q['method_id'] == 1){ 
        //         if(isset($msg_arr[3]) && ctype_xdigit($msg_arr[3])){ $q['pincode'] = Validator::hex_to_int($msg_arr[3]);}else{ return false;}
        //         if(isset($msg_arr[4]) && ctype_xdigit($msg_arr[4])){ $q['device_key'] = Validator::hex_to_int($msg_arr[4]);}else{ return false;}
        //         $q['data'] = '';
        //     }else{
        //         $data = '';
        //         unset($msg_arr[0]);unset($msg_arr[1]);unset($msg_arr[2]);
